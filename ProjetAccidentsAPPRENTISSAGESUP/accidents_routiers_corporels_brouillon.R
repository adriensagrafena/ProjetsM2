...
table_atm<-table(df$atm)

table_atm_grave<-table(df$grav,df$atm)
table_atm
table_grav
table_atm_grave
...
library("ggplot2")
a <- ggplot(df, aes(x = grav, fill = atm)) + 
  geom_bar(stat="count", position = "dodge") + ylab("Effectif") + 
...
b <- ggplot(df, aes(x = atm, fill = grav)) + 
  geom_bar(stat="count", position = "dodge") + ylab("Effectif") +
...
library('questionr')
# Profils lignes, pour comparer les modalités de la gravité
profil_ligne <- lprop(table_atm_grave, digits = 0, percent = TRUE)
profil_ligne
...
# Profils colonnes, pour comparer les modalités de la catégorie d'usagers
profil_colonne <- cprop(table_atm_grave, digits = 0, percent = TRUE)
profil_colonne
...
# on enlève num_accidents, commune et id_vehicule; on convertit la date en jour de la semaine et les heures
# par tranche horaire
library("lubridate")
...
ab <- which(df$vma<10 | df$vma==12 | df$vma==42 | df$vma>130)
df <- df[-c(ab),]
df$vma <- as.factor(df$vma)
df$grav[df$grav=="3"]<-"2"
df$grav<-droplevels.factor(df$grav)
df$nbv<-as.factor(df$nbv)
...
dtf_train<-dtf[train_id,]
dtf_test<-dtf[-train_id,]

#1er essai avec modèle complet, trivial et réduit AIC10000 pour voir
modele.complet<-multinom(formula = grav ~ .,data = dtf_train, model = TRUE, maxit = 600) 
pred.moda.modele.complet <- predict(object = modele.complet, newdata = dtf_test)
erreur.modele.complet <- mean(!(pred.moda.modele.complet == dtf_test$grav))
erreur.modele.complet #0.3338428



# z value de wald....pour le test de significativité des coeffs avec stats N(0;1)
# evidemment avec broom
library(broom)
tidy(modele.complet, conf.int= True, conf.level = 0.95, exponentiate = T)
# très lent et valeurs de Z très faibles as expected


modele.complet<-vglm(formula=grav~.,multinomial,dtf_train)
lrtest_vglm(modele.complet)

# statistiques du chi2 57431 avec chi² pvalue hyper faible



modele.reduit1<-multinom(formula = grav ~ catv + catu + obsm + agg + col + obs + 
                           catr + sexe + manv + trajet + circ + situ + nbv + vma, data = dtf, model = TRUE, maxit = 600)

pred.moda.modele.reduit1 <- predict(object = modele.reduit1 , newdata = dtf_test)
erreur.modele.reduit1 <- mean(!(pred.moda.modele.reduit1 == dtf_test$grav))
erreur.modele.reduit1

#0.3348362

modele.reduit1bis<-vglm(formula = grav ~ catv + catu + obsm + agg + col + obs + 
                      catr + sexe + manv + trajet + circ + situ + nbv + vma, data = dtf,multinomial,dtf_train)
lrtest_vglm(modele.reduit1bis)




# idem avec regroupement des modalités "hospitalisé" et "tué" avec
# modification du dataframe qui devient
# Nombre d'usagers par niveau de gravité
# 1 =  Indemne  
# 2 = Tué  
# 3 = Blessé hospitalisé  
...
# test modèle optimisé sur base de test et estimation erreur classification par crossvalidation
# par défaut 70/30 dans le partage entraînement/test


n<-nrow(df)

errclass<-function(l=0.7){
  train_id<-sample(1:n,size=l*n,replace=FALSE)
  data_train<-df[train_id,]
  data_test<-df[-train_id,]
  modele.complet <- multinom(formula = grav ~ catv + catu + obsm + agg + col + obs + 
                               catr + sexe + manv + trajet + circ + situ + nbv + vma, data = data_train, model = TRUE, maxit = 500)
  pred.moda.modele.complet <- predict(object = modele.complet, newdata = data_test)
  erreur.modele.complet <- mean(!(pred.moda.modele.complet == data_test$grav))
  return(erreur.modele.complet)
}

# ceci ne fournit qu'une valeur estimée de l'erreur (ê dans le cours)
# si on veut s'approcher de l'erreur de prédiction il faut répéter ceci et prendre
# la moyenne (LGN); on parallélise car c'est hyper long... même pour 50....
...
resultats<-foreach(e=1:10,.packages=c("nnet"),.combine=c) %dopar% {errclass(0.7)}
print(mean(resultats))     

#0.3361073   pour 10 répétitions
#0.3362665   pour 20 répétitions
#0.3358925   pour 30 répétitions
#0.3360844   pour 40 répétitions


resultats<-foreach(e=1:10,.packages=c("nnet"),.combine=c) %dopar% {errclass(0.8)}
print(mean(resultats))

#0.3357443   pour 10 répétitions
#0.3365047   pour 20 répétitions
#0.3355447   pour 40 répétitions
...
# bilan: grav ~ catv + catu + obsm + agg + col + obs + catr + sexe + manv + trajet + circ + situ + nbv + vma
# formule obtenue pour N= 10 000 pris au hasard conservée... 66,4% de bonnes prédictions par CV

library(MASS)

# sélection modèle optimal par AIC uniquement en méthode forward (ou backward?) vu le volume de données et variables
#la méthode forward selection  si faisable vu monstruosité des calculs

modele.forward <- step(object = modele.trivial, 
                       scope = list(lower = grav ~ 1, upper = formula(modele.complet$model)), 
...
# sélection modèle optimal par BIC uniquement en méthode forward (ou backward?) vu le volume de données et variables
#la méthode forward selection  si faisable vu monstruosité des calculs
...
set.seed(123)
train_id<-sample(1:n,size=0.7*n,replace=FALSE)
dfacm_train<-dfacm[train_id,]
dfacm_test<-dfacm[-train_id,]


modele.acm<-multinom(formula = grav ~ catr+vma+agg+obsm+circ+col+manv+lum+int+obs,data = dfacm_train, model = TRUE, maxit = 600) 

pred.moda.modele.acm <- predict(object = modele.acm, newdata = dfacm_test)
erreur.modele.acm <- mean(!(pred.moda.modele.acm == dfacm_test$grav))
erreur.modele.acm    #0.4786...


# modèle avec 10000 et test
n<-nrow(dtf)
set.seed(489)
x<-sample(1:n,size=10000,replace=FALSE)
dtfd<-dtf[x,]

set.seed(123)
y<-sample(1:10000, size=7000, replace=FALSE)
dtfd_train<-dtf[y,]
dtfd_test<-dtf[-y,]

modele.completvg<-vglm(formula=grav~.,multinomial,dtfd_train)
lrtest_vglm(modele.completvg)
# chi2 test log vraisemblance
#1 13544 -4839.3                          
#2 13998 -7266.7 454 4854.8  < 2.2e-16 ***

pred.moda.modele.complet <- predict(object = modele.complet, newdata = dtfd_test)
erreur.modele.complet <- mean(!(pred.moda.modele.complet == dtfd_test$grav))
erreur.modele.complet
